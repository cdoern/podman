...
func ToLibpodFilters(f urlP.Values) (filters []string) {
	for k, v := range f {
...
func ToURLValues(f []string) (filters urlP.Values) {
	filters = make(urlP.Values)
	for _, v := range f {
...
func ExecuteTransfer(args []string, parentFlags []string, quiet bool) (*entities.ImageLoadReport, *entities.ImageScpOptions, *entities.ImageScpOptions, []string, error) {
...
func ValidateAndConfigure(uri *urlP.URL, iden string) (*ssh.ClientConfig, error) {
